...
ANYSCALE_API_KEY = os.getenv('ANY_SCALE_API')
...
def get_equation_from_prompt(model, question, prompt, max_tokens, stop_token, temperature):
    # client = OpenAI(
...
def get_declarative_peano_equations(equations):
    result = equations
    print("response",result)
    eq_list = re.findall(r'\[\[.*?\]\]', result)
    print("re findall",eq_list)
    if len(eq_list) > 0:            
    ...
    try:
        ...
        if len(equation_list) == 1:
            try:
                ...
                print("goal_expression after parse", goal_expression)
                return float(sympify(goal_expression))
            except Exception as e:
                return 'invalid equations'
        print("goal_var", goal_var)
        if goal_var == None:
        ...
        for i in range(len(equation_list) - 1):
            ...
            if '?' not in sub_eqs:
                try:    
                    ...
                    print("sub_eqs after parse", sub_eqs)
                    sub_eqs = sympify(sub_eqs)
                except Exception as e:
                    return 'invalid equations'
                goal_expression_list.append(sub_eqs)
                print("goal_expression_list_sub", goal_expression_list)
                try:
                    try:
                        print("goal_expression_list_solve", goal_expression_list)
